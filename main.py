...
# p.resetDebugVisualizerCamera(1, 0, 0, cameraPos)
...
def debug():
    while 1:
        ...
        elif p.getKeyboardEvents() == {99: 4}:
            console.copen(globals(), locals())
        elif p.getKeyboardEvents() == {114: 1}:
            # Enter r to trigger real time simulation
            useRealTimeSimulation = True
            # Press t to pause real time simulation
        elif p.getKeyboardEvents() == {116: 1}:
            useRealTimeSimulation = False
            # Press j to initiate jump
